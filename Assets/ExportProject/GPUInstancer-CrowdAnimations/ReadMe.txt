GPU Instancer - Crowd Animations v1.1.9
Copyright ©2019-24 GurBu Technologies
---------------------------------
...
IMPORTANT: Requires GPU Instancer v1.8.3 or later

v1.1.9

New: Added a new URP Lit shader with Specular setup.

Fixed: Shader Graph Crowd Setup node now correctly uses the Tangent vector in Object space instead of World space.

v1.1.8

Changed: Replaced absolute methods to support Unity 6.

Fixed: Animation events not being executed at times when the event is placed at the beginning or end of the animation clip.

v1.1.7

Changed: Exposed Remove Bone GOs option when using Crowd Animator

Fixed: NoGameObjectsWithJobs demo script does not check for clip loop setting
Fixed: Incorrect animation weights when a new animation starts during a transition
