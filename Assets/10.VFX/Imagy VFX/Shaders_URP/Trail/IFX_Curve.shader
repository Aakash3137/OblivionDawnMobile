...
Shader "QFX/IFX/Trail/Curve_URP"
{
    Properties
    {
        [HDR]_TintColor("Tint Color", Color) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Calculate texture offset based on particle animation
                float texOffsetX = _MainTexOffsetX + input.uv3.z;
                
                // Create texture offset vector
                float2 texOffset = float2((texOffsetX - _TexAddX), _MainTexOffsetY);
                
                // Calculate UV for main texture with scrolling
                float2 mainTexUV = input.uv * _MainTex_ST.xy + _MainTex_ST.zw;
                float2 pannerUV = mainTexUV - texOffset;
                
                // Animate texture with time
                float2 speedDir = float2(_Speed.x, _Speed.y);
                float2 animatedUV = pannerUV + speedDir * _Time.y;
                
                // Sample main texture
                half4 mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, animatedUV);
                
                // Create procedural mask based on UV coordinates
                float2 uv = input.uv;
                float maskX = step(abs(uv.x - texOffsetX), _MaskSize.x);
                float maskY = step(abs(uv.y - 0.5), _MaskSize.y);
                float proceduralMask = saturate(maskX * maskY);
                
                // Calculate final emission color
                half3 emission = mainTex.r * _TintColor.rgb * proceduralMask * input.color.rgb;
                
                // Calculate opacity
                float opacity = input.color.a * mainTex.r * proceduralMask;
                
                // Create UV-based mask for opacity modulation
                float y = uv.y;
                float x = uv.x;
                float uvMaskPart1 = y * (1.0 - y) * (1.0 - x) * 8.0;
                float uvMaskPart2 = (x * y * (1.0 - y)) * 20.0;
                float uvMask = uvMaskPart1 * uvMaskPart2;
                
                // Apply optional mask texture
                float finalAlpha = opacity;
                if (_UseMaskMap > 0.5)
                {
                    float2 maskUV = uv * _MaskMap_ST.xy + _MaskMap_ST.zw;
                    half maskTex = SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap, maskUV).r;
                    finalAlpha = opacity * maskTex;
                }
                else
                {
                    finalAlpha = uvMask * opacity;
                }
                ...
                // Combine result
                half4 result = half4(emission, finalAlpha);
