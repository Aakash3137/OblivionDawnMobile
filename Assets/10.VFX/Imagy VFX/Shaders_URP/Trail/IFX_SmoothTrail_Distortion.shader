...
Shader "QFX/IFX/Trail/SmoothTrail_Distortion_URP"
{
    Properties
    {
        _Cutoff("Mask Clip Value", Float) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Dissolve effect
                float2 noiseMapUV = input.uv * _NoiseMap_ST.xy + _NoiseMap_ST.zw;
                float noiseValue = SAMPLE_TEXTURE2D(_NoiseMap, sampler_NoiseMap, noiseMapUV).r;
                
                float dissolveProgress = 1.0 - (_DissolveAmount + input.uv4.z);
                ...
                // Main texture UV with scrolling
                float2 mainTexUV = input.uv * _MainTex_ST.xy + _MainTex_ST.zw;
                float2 scrollUV1 = float2(_Speed.x, _Speed.y);
                float2 pannerUV1 = mainTexUV + scrollUV1 * _Time.y;
                
                // Sample main texture
                half4 mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, pannerUV1);
                
                // Noise texture UV with scrolling
                float2 noiseTexUV = input.uv * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
                float2 scrollUV2 = float2(_Speed.z, _Speed.w);
                float2 pannerUV2 = noiseTexUV + scrollUV2 * _Time.y;
                
                // Sample noise texture
                half4 noiseTex = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, pannerUV2);
                
                // Create procedural mask based on UV
                float x = input.uv.x;
                float y = input.uv.y;
                
                // Fade mask based on UV.x
                float fadeMask = smoothstep(0.2, 0.8, (1.0 - x));
                fadeMask = saturate(fadeMask);
                
                // Trail shape mask
                float trailMaskY = y * (1.0 - y) * (1.0 - x) * 8.0;
                
                // Combine noise with fade mask
                float combinedMask = saturate((fadeMask + noiseTex.r) - x);
                
                // Distortion effect using screen texture
                float2 screenUV = input.screenPos.xy / input.screenPos.w;
                
                // Apply distortion based on main texture
                float2 distortion = mainTex.rg * _Distortion;
                ...
                half4 screenColor = SAMPLE_TEXTURE2D_X(_CameraOpaqueTexture, sampler_CameraOpaqueTexture, distortedUV);
                
                // Calculate emission
                half3 emission = mainTex.r * combinedMask * _TintColor.rgb * trailMaskY * input.color.rgb;
                half3 finalEmission = screenColor.rgb + emission;
                
                // Apply dissolve edge glow
                if (_DissolveEdgeWidth > dissolveFactor)
                {
                    finalEmission = _DissolveColor.rgb;
                }
                else
                {
                    finalEmission = screenColor.rgb + emission;
                }
                
                // Calculate alpha
                float trailMaskX = (x * y * (1.0 - y)) * 20.0;
                float alpha = input.color.a * trailMaskY * fadeMask * trailMaskX;
                alpha = saturate(alpha);
                
                // Apply fog
                finalEmission = MixFog(finalEmission, input.fogCoord);
                
                return half4(finalEmission, alpha);
            }
            ENDHLSL
        }
    ...
    CustomEditor "UnityEditor.Rendering.Universal.ShaderGUI.CustomTrailDistortionShaderGUI"
