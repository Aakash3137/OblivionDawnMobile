...
Shader "QFX/IFX/Trail/SmoothTrail_URP"
{
    Properties
    {
        _Cutoff("Mask Clip Value", Float) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Calculate emission
                half3 emission = mainTex.rgb * combinedMask * _TintColor.rgb * trailMaskY * input.color.rgb;
                ...
                {
                    emission = mainTex.rgb * combinedMask * _TintColor.rgb * trailMaskY * input.color.rgb;
                }
                
                // Calculate alpha
                float trailMaskX = (x * y * (1.0 - y)) * 20.0;
                float alpha = input.color.a * mainTex.r * noiseTex.r * trailMaskY * fadeMask * trailMaskY * trailMaskX;
                alpha = saturate(alpha);
                
                // Apply fog
                emission = MixFog(emission, input.fogCoord);
    ...
    CustomEditor "UnityEditor.Rendering.Universal.ShaderGUI.CustomTrailShaderGUI"
