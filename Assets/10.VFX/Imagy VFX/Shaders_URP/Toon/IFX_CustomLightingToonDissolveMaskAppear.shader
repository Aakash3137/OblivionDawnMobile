...
Shader "QFX/IFX/Toon/CustomLightingToonDissolveMaskAppear_URP"
{
    Properties
    {
        _Cutoff("Mask Clip Value", Float) = 0.5
        _ToonRamp("Toon Ramp", 2D) = "white" {}
        _DistanceScale("Distance Scale", Float) = 1.53
        [HDR]_DissolveColor("Dissolve Color", Color) = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                // Calculate distance-based alpha mask
                float4 objectOriginWS = mul(UNITY_MATRIX_M, float4(0, 0, 0, 1));
                float distanceToCenter = length(output.positionWS - objectOriginWS.xyz);
                float scaledDistance = distanceToCenter / _DistanceScale;
                float distanceAlphaMask = pow(scaledDistance, _DistancePower);
                output.alphaMask = 1.0 - saturate(distanceAlphaMask);
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Alpha from distance mask
                float alpha = input.alphaMask;
