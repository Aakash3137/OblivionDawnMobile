...
Shader "QFX/IFX/Cutout/CutoutAlignment_URP"
{
    Properties
    {
        [HDR]_TintColor("Tint Color", Color) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Attributes
            {
                ...
                float2 uv : TEXCOORD0;
                float3 uv3 : TEXCOORD2;   // uv3.z is required
                float4 color : COLOR;
            };
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float4 emission =
                    pow(mainTex, _TexPower) * _TintColor * i.color;
                ...
                // -------- Distance band --------
                float dist =
                    abs(axis + (_AlphaCutout + i.uv3.z)) - _Size;

                float smoothBand =
                    smoothstep(_NoiseMin, _NoiseMax, dist);

                float fade =
                    saturate(1.0 - dist);

                float opacityMask =
                    (noise - smoothBand) * fade;

                clip(saturate(opacityMask) - _Cutoff);
