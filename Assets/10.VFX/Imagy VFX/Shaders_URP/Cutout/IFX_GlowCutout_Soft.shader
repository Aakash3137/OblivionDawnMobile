...
Shader "QFX/IFX/Cutout/GlowCutout_Soft_URP"
{
    Properties
    {
        ...
        _AlphaCutout("Alpha Cutout", Float) = 0
        [HDR]_GlowColor("Glow Color", Color) = (0,0,0,0)
        _NoiseAdjust("Noise Adjust", Range(0,0.2)) = 0.1
        [HDR]_TintColor("Tint Color", Color) = (0,0,0,0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float alphaBase =
                    lerp(_GlowColor.a * i.color.a,
                         i.color.a,
                         invFresnel);

                float2 noiseUV =
                    i.uv2 * _NoiseTex_ST.xy + _NoiseTex_ST.zw;

                float noise =
                    SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, noiseUV).r;

                float dissolve =
                    smoothstep(0.0, _NoiseAdjust,
                        noise - (_AlphaCutout + i.uv3.z));

                float alpha = alphaBase * dissolve;
