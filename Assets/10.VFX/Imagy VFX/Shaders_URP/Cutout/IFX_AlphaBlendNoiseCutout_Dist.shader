...
Shader "QFX/IFX/Cutout/AlphaBlendNoiseCutout_Dist_URP"
{
    Properties
    {
        [HDR]_TintColor("Tint Color", Color) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float alpha = saturate(emission.a * i.color.a);

                // -------- Flow map --------
                float2 flowUV = TRANSFORM_TEX(i.uv, _FlowMap);
                float flowSample = SAMPLE_TEXTURE2D(_FlowMap, sampler_FlowMap, flowUV).r * _Flow;

                float2 noiseUV = flowSample + (flowUV + time * _FlowSpeed);
                ...
                float cutout =
                    (mainR -
                    (noise - (1.0 - (_AlphaCutout + i.uv3.z))))
                    * mainTex.a;

                clip(cutout - _Cutoff);

                return half4(emission.rgb, alpha);
            }
            ENDHLSL
        }
    }
