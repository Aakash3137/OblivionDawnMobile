...
Shader "QFX/IFX/Cutout/GlowCutout_URP"
{
    Properties
    {
        _Cutoff("Mask Clip Value", Float) = 0.1
        _FresnelScale("Fresnel Scale", Range(0,1)) = 0.51
        _FresnelPower("Fresnel Power", Range(0,5)) = 2
        _NoiseTex("Noise Tex", 2D) = "white" {}
        _AlphaCutout("Alpha Cutout", Float) = 0
        [Toggle]_CutoutFresnel("Cutout Fresnel", Float) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Attributes
            {
                ...
                float2 uv0        : TEXCOORD0; // MainTex
                float2 uv1        : TEXCOORD1; // Noise
                float3 uv3        : TEXCOORD2; // uv3.z bias
                float4 color      : COLOR;
            };
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float fresnel =
                    saturate(_FresnelScale * pow(1.0 - ndotv, _FresnelPower));

                float invFresnel = saturate(1.0 - fresnel);

                // -------- Main texture (panner) --------
                float2 uvMain =
                    i.uv0 * _MainTex_ST.xy + _MainTex_ST.zw +
                    _Time.y * _MainSpeed;

                float4 mainTex =
                    SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uvMain);

                // -------- Color / emission --------
                float4 tintPart =
                    _TintColor * i.color * fresnel;

                float4 colorMix =
                    lerp(_GlowColor, tintPart, fresnel);

                float3 emission =
                    mainTex.rgb * colorMix.rgb;

                // -------- Alpha (not cutout) --------
                float alpha =
                    lerp(_GlowColor.a, _TintColor.a, fresnel) *
                    i.color.a;

                // -------- Noise cutout --------
                float2 noiseUV =
                    i.uv1 * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
                ...
                float noiseStep =
                    smoothstep(0.0, 0.2,
                        noise - (_AlphaCutout + i.uv3.z));

                float clipValue =
                    lerp(invFresnel + noiseStep,
                         invFresnel * noiseStep,
                         _CutoutFresnel);
