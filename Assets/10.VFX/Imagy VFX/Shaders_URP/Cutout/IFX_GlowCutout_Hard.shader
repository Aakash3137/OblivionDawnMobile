...
Shader "QFX/IFX/Cutout/GlowCutout_Hard_URP"
{
    Properties
    {
        _NoiseTex("Noise Tex", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // -------- Mask / glow blend --------
                float2 maskUV = TRANSFORM_TEX(i.uv, _Mask);
                float maskR = SAMPLE_TEXTURE2D(_Mask, sampler_Mask, maskUV).r;

                float maskStep =
                    smoothstep(_AdjustMin, _AdjustMax, maskR);

                float invMask = saturate(1.0 - maskStep);

                float4 tintPart =
                    _TintColor * i.color * maskStep;

                float4 glowPart =
                    _GlowColor;

                float4 finalColor =
                    lerp(tintPart, glowPart, invMask);

                // -------- Alpha blend --------
                float glowAlpha =
                    lerp(i.color.a,
                         _GlowColor.a * i.color.a,
                         invMask);
