...
Shader "QFX/IFX/Abilities/SimpleProjectileTrail_URP"
{
    Properties
    {
        _TimeScale("Time Scale", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                // ----- Flow map -----
                float2 flowUV = TRANSFORM_TEX(i.uv, _FlowMap);
                flowUV += time * float2(-1.0, 0.0);

                float2 flowSample = SAMPLE_TEXTURE2D(_FlowMap, sampler_FlowMap, flowUV).rg;
                flowSample = (flowSample - 0.5) * 2.0;
                flowSample *= _FlowStrength;

                // ----- Main texture -----
                float2 mainUV = TRANSFORM_TEX(i.uv, _MainTex) + flowSample;
                float mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainUV).r;

                // ----- Mask -----
                float2 maskUV = TRANSFORM_TEX(i.uv, _MaskMap);
                float mask = SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap, maskUV).r;

                // ----- Emission & Alpha -----
                float emissionFactor = mainTex * mask;
                float3 emission = _TintColor.rgb * emissionFactor * i.color.rgb;
                float alpha = emissionFactor * i.color.a;
