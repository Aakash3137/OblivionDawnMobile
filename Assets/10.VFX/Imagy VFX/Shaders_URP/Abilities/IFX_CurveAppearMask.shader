...
Shader "QFX/IFX/Abilities/CurveAppearMask_URP"
{
    Properties
    {
        _Cutoff("Mask Clip Value", Float) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float2 flowPanner = flowUV + time * _FlowMapSpeed;

                float2 flowSample = SAMPLE_TEXTURE2D(_FlowMap, sampler_FlowMap, flowPanner).rg;
                flowSample = (flowSample - 0.5) * 2.0;

                float flowMask = flowUV.x * (1.0 - flowUV.x);
                float2 flowOffset = flowSample * _FlowStrength * flowMask;

                // Main texture
                float2 mainUV = i.uv + flowOffset;
                float mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainUV).r;

                // Emission color
                float3 emission = _TintColor.rgb * mainTex * i.color.rgb;

                // Alpha mask
                float2 maskUV = i.uv * _MaskMap_ST.xy + _MaskMap_ST.zw;
                float mask = SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap, maskUV).r;

                float alpha = saturate(mainTex * i.color.a * mask);
                ...
                float remapX = lerp(-0.5, 0.5, i.uv.x);
                float appear = 1.0 - (_MaskAppearProgress + i.uv3.z);
                float clipVal = abs(remapX + appear) - _Size;

                clip(noise - clipVal - _Cutoff);
