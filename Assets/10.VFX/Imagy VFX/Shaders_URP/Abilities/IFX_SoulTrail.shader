...
Shader "QFX/IFX/Abilities/SoulTrail_URP"
{
    Properties
    {
        _Cutoff("Mask Clip Value", Float) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_Head);         SAMPLER(sampler_Head);
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                // -------- Dissolve --------
                float2 dissolveUV = TRANSFORM_TEX(i.uv, _DissolveMap);
                float dissolveTex = SAMPLE_TEXTURE2D(_DissolveMap, sampler_DissolveMap, dissolveUV).r;

                float dissolveRemap = dissolveTex +
                    lerp(-0.6, 0.6, 1.0 - _DissolveAmount);

                float dissolveMask = saturate(dissolveRemap);

                clip(dissolveMask - _Cutoff);

                // -------- Head --------
                float2 headUV = TRANSFORM_TEX(i.uv, _Head);
                float headAlpha = SAMPLE_TEXTURE2D(_Head, sampler_Head, headUV).a;

                // -------- Flow Map --------
                float2 baseUV = TRANSFORM_TEX(i.uv, _MainTex);
                float2 flowUV = baseUV + time * _FlowMapSpeed;

                float2 flow = SAMPLE_TEXTURE2D(_FlowMap, sampler_FlowMap, flowUV).rg;
                flow = (flow - 0.5) * 2.0 * _FlowStrength;

                // -------- Main Texture --------
                float2 mainUV = baseUV + flow + time * _MainTexSpeed;
                float mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainUV).r;

                // -------- Mask --------
                float mask = SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap,
                    TRANSFORM_TEX(i.uv, _MaskMap)).r;

                // -------- Color Blend --------
                float4 colorLerp = lerp(_ColorStart, _ColorEnd, 0);
                float3 baseEmission =
                    (_ColorStart * headAlpha).rgb +
                    (colorLerp.rgb * mainTex * mask * _FinalPower);

                float3 emission =
                    (_DissolveEdgeWidth > dissolveMask)
                    ? _DissolveColor.rgb
                    : baseEmission;

                float alpha = saturate(
                    headAlpha + (mainTex * mask * i.color.a)
                );
