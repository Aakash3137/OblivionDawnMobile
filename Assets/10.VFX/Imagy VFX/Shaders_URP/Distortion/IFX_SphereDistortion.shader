...
Shader "QFX/IFX/Distortion/URP/SphereDistortion"
{
    Properties
    {
        _FresnelPower("Fresnel Power", Float) = 2.49
        ...
        _NoiseSpeed("Noise Speed", Vector) = (0,0,0,0)
        _UseFresnelOpacity("Use Fresnel Opacity", Float) = 0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // URP opaque texture for screen grab
            ...
            {
                // Fresnel factor
                float fresnel = _FresnelScale * pow(1.0 - saturate(dot(IN.worldNormal, IN.viewDir)), _FresnelPower);
                float fresnelMix = fresnel * (1.0 - fresnel);

                // Noise offset for distortion
                float2 uvNoise = IN.uv * _NoiseMap_ST.xy + _NoiseMap_ST.zw;
                uvNoise += _Time.y * _NoiseSpeed;
                float noiseSample = tex2D(_NoiseMap, uvNoise).r;
                ...
                float3 normalView = normalize(mul((float3x3)UNITY_MATRIX_V, IN.worldNormal));
                float3 dirView = normalize(mul((float3x3)UNITY_MATRIX_V, IN.viewDir));
                float3 offsetDir = normalize(normalView - dirView);
                screenUV += noiseSample * fresnelMix * _Distortion * IN.color.rgb.xy * offsetDir.xy;

                // Sample screen color
                float4 screenCol = SAMPLE_TEXTURE2D(_CameraOpaqueTexture, sampler_CameraOpaqueTexture, screenUV);

                // Output
                float alpha = IN.color.a * lerp(1.0, fresnelMix, _UseFresnelOpacity) * _TintColor.a;
                float3 finalRGB = screenCol.rgb * _TintColor.rgb;
