...
Shader "QFX/URP/Distortion/DistortionFresnel_Unity6"
{
    Properties
    {
        _MainTex("Main Tex", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                // ---- Distortion normal ----
                float2 distUV =
                    TRANSFORM_TEX(i.uv, _DistortionTexture) * _Scale +
                    _Time.y * _DistortionSpeed;

                float3 distNormal =
                    UnpackNormal(
                        SAMPLE_TEXTURE2D(_DistortionTexture, sampler_DistortionTexture, distUV)
                    );

                float2 distortionOffset = distNormal.xy * _Distortion;
                ...
                float3 sceneColor =
                    SAMPLE_TEXTURE2D(_CameraOpaqueTexture, sampler_CameraOpaqueTexture, screenUV).rgb;

                // ---- Main texture ----
                float3 baseTex =
                    SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv).rgb;

                float3 color = (_TintColor.rgb * baseTex) + sceneColor;

                // ---- Fresnel ----
                float NdotV = dot(i.normalWS, i.viewDirWS);
                float fresnel =
                    _FresnelScale *
                    pow(1.0 - saturate(NdotV), _FresnelPower);

                // ---- Alpha ----
                float baseAlpha = i.color.a * _TintColor.a;
                float alpha = lerp(
                    baseAlpha,
                    baseAlpha * saturate(fresnel),
                    _UseFresnelAlpha
                );
