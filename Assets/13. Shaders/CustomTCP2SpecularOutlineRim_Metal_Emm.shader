...
Shader "Toony Colors Pro 2/Custom/TCP2SpecularOutlineRim_Metal_Emm"
{
	Properties
	{
		...
		[Toggle(TCP2_EMISSION)] _UseEmission ("Enable Emission", Float) = 0
		[HDR] _EmissionColor ("Emission Color", Color) = (1,1,1,1)
		_EmissionStrength ("Emission Strength", Range(0,1)) = 0.0
	...
	SubShader
	{
		...
		TCP2_TEX2D_WITH_SAMPLER(_MetalnessMap);  // Added Metalness Map
		TCP2_TEX2D_WITH_SAMPLER(_EmissionMap);   // Added Emission Map
		...
		CBUFFER_START(UnityPerMaterial)
			...
			// Added Metalness Properties
			float _Metalness;
			float4 _MetalnessMap_ST;
			
			// Added Emission Properties
		...
		ZWrite [_ZWrite]
			...
			#pragma shader_feature_local TCP2_EMISSION  // Added Emission keyword
		#pragma shader_feature_local _ _ALPHAPREMULTIPLY_ON
			...
			struct Varyings
			{
				...
				float2 pack2 : TEXCOORD5; /* pack2.xy = metalnessMap UV */
				float2 pack3 : TEXCOORD6; /* pack3.xy = emissionMap UV */
				UNITY_VERTEX_INPUT_INSTANCE_ID
				UNITY_VERTEX_OUTPUT_STEREO
			};
			...
			Varyings Vertex(Attributes input)
			{
				...
				output.pack2.xy = input.texcoord0.xy * _MetalnessMap_ST.xy + _MetalnessMap_ST.zw;  // Metalness UV
				output.pack3.xy = input.texcoord0.xy * _EmissionMap_ST.xy + _EmissionMap_ST.zw;   // Emission UV
			...
			) : SV_Target
			{
				...
				// Added Metalness Sampling
				float __metalness = ( _Metalness );
				float __metalnessMap = ( TCP2_TEX2D_SAMPLE(_MetalnessMap, _MetalnessMap, input.pack2.xy).r );
				float metalness = __metalness * __metalnessMap;
				
				// Added Emission Sampling
				float3 __emissionColor = ( _EmissionColor.rgb );
				float __emissionStrength = ( _EmissionStrength );
				float3 __emissionMap = ( TCP2_TEX2D_SAMPLE(_EmissionMap, _EmissionMap, input.pack3.xy).rgb );
				float3 emissionMapColor = __emissionMap * __emissionColor * __emissionStrength;
				...
				// Blend specular color with albedo based on metalness
				float3 finalSpecularColor = lerp(__specularColor, albedo, metalness);
				#else
				float3 finalSpecularColor = __specularColor;
				#endif
				...
				//Apply specular with metalness
				emission.rgb += spec * lightColor.rgb * finalSpecularColor;
				#endif
				...
				// Apply emission
				#if defined(TCP2_EMISSION)
				emission.rgb += emissionMapColor;
				#endif
...
/* TCP_DATA u config(ver:"2.9.18";unity:"2022.3.17f1";tmplt:"SG2_Template_URP";features:list["UNITY_5_4","UNITY_5_5","UNITY_5_6","UNITY_2017_1","UNITY_2018_1","UNITY_2018_2","UNITY_2018_3","UNITY_2019_1","UNITY_2019_2","UNITY_2019_3","UNITY_2019_4","UNITY_2020_1","UNITY_2021_1","UNITY_2021_2","UNITY_2022_2","OUTLINE","OUTLINE_URP_FEATURE","OUTLINE_CONSTANT_SIZE","SPEC_PBR_GGX","SPECULAR","AUTO_TRANSPARENT_BLENDING","RIM","RIM_LIGHTMASK","RIM_SHADER_FEATURE","SPECULAR_SHADER_FEATURE","RIM_VERTEX","OUTLINE_CLIP_SPACE","TEMPLATE_LWRP"];flags:list[];flags_extra:dict[];keywords:dict[RENDER_TYPE="Opaque",RampTextureDrawer="[TCP2Gradient]",RampTextureLabel="Ramp Texture",SHADER_TARGET="3.0",RIM_LABEL="Rim Lighting"];shaderProperties:list[];customTextures:list[];codeInjection:codeInjection(injectedFiles:list[];mark:False);matLayers:list[]) */
/* TCP_HASH c9da7e73e6d97fc661d04063739c393c */